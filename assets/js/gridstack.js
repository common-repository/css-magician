  ...
  */
  ...
  GridStackEngine.prototype._fixCollisions = function(node) {
   var self = this;
    this._sortNodes(-1);
    var nn = node;
  //  var disableCollision = this.nodes[0]._grid.opts.disableCollisionClass;
  ...
  GridStackEngine.prototype.moveNode = function(node, x, y, width, height, noPack) {
    ...
    if(!nodeElement.hasClass(disableCollision))   
    this._fixCollisions(node);
  ...
  var GridStack = function(el, opts) {
    ...
    this.opts = Utils.defaults(opts, {
      ...
      ddPlugin: null, 
      disableCollisionClass:'disableCollision'    
    });
  ...
  GridStack.prototype._prepareElementsByNode = function(el, node) {
    ...
    var onEndMoving = function(event, ui) {
      var o = $(this);
      var gridstackID = self.el.id;
      var originalWidth = $(event.target).css('width');
      var originalHeight =  $(event.target).css('height');
      var parentWidth = self.el.clientWidth;
      var parentHeight = self.el.clientHeight;
      ...
      node.el = o.get(0);
      self.placeholder.hide();
      var disableCollision = self.opts.disableCollisionClass;
      ...
      if (event.type === 'resizestop') {
        ...
        if(typeof(disableCollision)!='undefined' && $(event.target).hasClass(disableCollision))
        {
          var realWidth = ui.size.width/parentWidth*100+"%";
          var realHeight = ui.size.height/parentHeight*100+"%";       
          var realLeft = ( 100 * parseFloat(ui.position.left / parseFloat(parentWidth)) ) + "%" ;
          var realTop = ( 100 * parseFloat(ui.position.top / parseFloat(parentHeight)) ) + "%" ; 

            o.css({'height':realHeight,'width':realWidth,'top':realTop,'left':realLeft}); 
            o.attr('data-columnrealtop',realTop).attr('data-columnrealleft',realLeft);             
        }    
      }
      if (typeof(disableCollision)!='undefined' && event.type === 'dragstop' && $(event.target).hasClass(disableCollision)) {
         var realLeft = ( 100 * parseFloat(ui.position.left / parseFloat(parentWidth)) ) + "%" ;
         var realTop = ( 100 * parseFloat(ui.position.top / parseFloat(parentHeight)) ) + "%" ;
         var realWidth = event.target.style.width;
         var realHeight = event.target.style.height;
          
          o.css({'height':realHeight,'width':realWidth,'top':realTop,'left':realLeft}); 
         o.attr('data-columnrealtop',realTop).attr('data-columnrealleft',realLeft);
      }
   $('#'+gridstackID+' .gridstack-save').show();
